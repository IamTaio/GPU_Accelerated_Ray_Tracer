...
__device__ __host__ double random_double(seed_t seed) {
...
__device__ __host__ double random_double(double min, double max, seed_t seed) {
	return min + (max - min) * random_double(seed);
}

__device__ __host__ int random_int(int min, int max, seed_t seed) {
	#ifdef __CUDA_ARCH__
