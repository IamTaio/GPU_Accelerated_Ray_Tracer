...
__host__ __device__ inline double random_double(seed_t seed) {
...
__host__ __device__ inline double random_double(double min, double max, seed_t seed) {
	return min + (max - min) * random_double(seed);
}

__host__ __device__ inline int random_int(int min, int max, seed_t seed) {
	#ifdef __CUDA_ARCH__
