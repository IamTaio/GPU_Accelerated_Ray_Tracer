...
public:
	...
	__device__  vec3(double e0, double e1, double e2) : e{ e0, e1, e2 } {}
	 ...
	 __device__ static vec3 random(seed_t* seed) {
		return vec3(random_double(seed), random_double(seed), random_double(seed));
	}

	 __device__ static vec3 random(double min, double max, seed_t* seed) {
...
using point3 = vec3;
 ...
 __device__ inline vec3 random_unit_vector(seed_t* seed) {
 ...
 __device__ inline vec3 random_on_hemisphere(const vec3& normal, seed_t* seed) {
 ...
 __device__ inline vec3 random_in_unit_disk(seed_t* seed) {
	while (true) {
