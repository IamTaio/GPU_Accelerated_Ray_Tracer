...
public:
    ...
    constexpr __device__ aabb(const interval& x, const interval& y, const interval& z)
        : x(x), y(y), z(z) {
    }
    ...
    __device__ interval  operator[](int i) const { return i == 1 ? y : i == 2 ? z : x; }
    __device__ interval& operator[](int i) { return i == 1 ? y : i == 2 ? z : x; }
    ...
    // FIX: static methods instead of static const members
